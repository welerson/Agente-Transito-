...
export default function App() {
  ...
  // MOTOR DE EXTRAÇÃO MBFT 5.0 - ESPECIALIZADO EM DUAS COLUNAS
  const parseManualText = (rawText: string): Partial<Infraction> | null => {
    try {
      ...
      // ALGORITMO DE SEPARAÇÃO DE COLUNAS (PARSER 5.0)
      const processTechnicalSection = () => {
        ...
        lines.forEach(line => {
          ...
          if (upper.includes("QUANDO AUTUAR") && upper.includes("QUANDO NÃO AUTUAR")) {
            // Se ambos na mesma linha, o bucket é o 0 e o processador de linha vai splitar
            activeBucket = 0;
            return;
          }
          ...
          // LÓGICA DE SPLIT DE COLUNA (Detecta 1. ... 1. ...)
          // Se a linha tem dois números iguais, ou dois números em sequência, splita
          const items = line.split(/(\s\d+[\s\.\)]+)/);
          
          if (items.length >= 3 && activeBucket === 0) {
            // Linha com múltiplas colunas detectada
            // items[0] = texto item 1, items[1] = numeração 2, items[2] = texto item 2
  ...
  const processPDF = async (file: File) => {
    ...
    setProgress(0);
    setBatchStatus('Iniciando análise inteligente...');
    try {
      ...
      alert(`SUCESSO: ${importedCount} infrações configuradas com diretrizes detalhadas.`);
              ...
              <div className="space-y-4">
                <div className="bg-green-50 p-6 rounded-[2rem] border border-green-100">
                  ...
                  <ul className="text-xs font-bold text-green-900 space-y-4">
                    {selectedInfraction.quando_atuar?.length ? selectedInfraction.quando_atuar.map((t, i) => <li key={i} className="flex gap-2 leading-relaxed"><span>•</span> <span>{t}</span></li>) : <li className="opacity-40 italic">Sem diretrizes disponíveis</li>}
                  </ul>
                </div>
                <div className="bg-red-50 p-6 rounded-[2rem] border border-red-100">
                  ...
                  <ul className="text-xs font-bold text-red-900 space-y-4">
                    {selectedInfraction.quando_nao_atuar?.length ? selectedInfraction.quando_nao_atuar.map((t, i) => <li key={i} className="flex gap-2 leading-relaxed"><span>•</span> <span>{t}</span></li>) : <li className="opacity-40 italic">Sem restrições listadas</li>}
                  </ul>
                </div>
                
                {selectedInfraction.definicoes_procedimentos && selectedInfraction.definicoes_procedimentos.length > 0 && (
                  <div className="bg-blue-50 p-6 rounded-[2rem] border border-blue-100">
                    <h4 className="text-[10px] font-black text-blue-700 uppercase mb-4 tracking-widest">Procedimentos MBFT</h4>
